...
class MainViewModel(private val repository: LocalRepository = LocalRepositoryImpl(App.getEntityDao())) :
    ViewModel() {
    ...
    private fun getDataFromLocalSource() {
        ...
        Thread {
            ...
            liveDataToObserve.postValue(
                AppState.Success(
                    repository.getLocalMainAnimeList().sortedBy { if (isRu) it.ruTitle else it.enTitle }
                )
            )
        }.start()
    }
