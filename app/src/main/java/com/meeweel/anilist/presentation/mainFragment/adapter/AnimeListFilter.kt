...
data class AnimeListFilter(
    ...
    private var genre: Genre = DEFAULT_GENRE_TEXT,
    private var yearFrom: Int = DEFAULT_YEAR_FROM,
    private var yearTo: Int = DEFAULT_YEAR_TO,
    private var sort: Sort = DEFAULT_SORT,
) {
    ...
    fun filter(list: List<ShortAnime>): List<ShortAnime> {
        ...
        if (genre != DEFAULT_GENRE_TEXT) {
            ...
            for (item in newList) {
                if (item.enGenre.replaceAfter(genre.textName, "")
                        .replaceBefore(genre.textName, "") == genre.textName
                ) alterList.add(item)
            }
            newList = alterList
        }
    ...
    private fun findByWord(list: List<ShortAnime>): List<ShortAnime> {
        return if (titleText == DEFAULT_TITLE_TEXT) {
            ...
            for (item in list) {
                if (item.enTitle.lowercase().replaceAfter(titleText.lowercase(), "")
                        .replaceBefore(titleText.lowercase(), "") == titleText.lowercase()
                    || item.ruTitle.lowercase().replaceAfter(titleText.lowercase(), "")
                        .replaceBefore(titleText.lowercase(), "") == titleText.lowercase()
                ) {
                    newList.add(item)
                }
    ...
    enum class Sort(val textName: String) {
        ALPHABET("Alphabet"), ALPHABET_REVERS("Alphabet reverse"), RATING("Rating"), NEW("New"), OLD(
            "Old"
        )
    }
    ...
    enum class Genre(val textName: String, val pos: Int) {
        ...
        FANTASY("Fantasy", 4), DRAMA("Drama", 5), ACTION("Action", 6), ROMANTIC(
            "Romantic",
            7
        ),
        SPACE("Space", 8), HORROR("Horror", 9), WESTERN("Western", 10), GAME(
            "Game",
            11
        ),
        MYSTERY("Mystery", 12), SPORTS("Sports", 13),
        PSYCHOLOGICAL_THRILLER("Psychological Thriller", 14), CRIME(
            "Crime",
            15
        ),
        ADVENTURE("Adventure", 16), DARK_FANTASY(
            "Dark Fantasy",
            17
        ),
        SCIENCE_FICTION("Science Fiction", 18), MECHA(
            "Mecha",
            19
        ),
        ALTERNATE_HISTORY("Alternate History", 20),
        STEAMPUNK("Steampunk", 21), ROMANCE("Romance", 22), MILITARY(
            "Military",
            23
        ),
        SCI_FI("Sci-Fi", 24), SHOUNEN("Shounen", 25), SHOUJO("Shoujo", 26), DEMONS(
            "Demons",
            27
        ),
        SCHOOL("School", 28), SAMURAI("Samurai", 29), HAREM("Harem", 30), KIDS("Kids", 31),
